                ...
                if (sEntry.Length > 0)
                {
                    ...
                    switch (sEntry[0].ToString())
                    {
                        case NonInvestmentAccountFields.Date:
                            // Set the date value
                            bt.Date = sEntry.Substring(1).ParseDateString(config);

                            // Stop processing
                            break;
                        case NonInvestmentAccountFields.Amount:
                            // Set the amount value
                            bt.Amount = sEntry.Substring(1).ParseDecimalString(config);
                        ...
                        case NonInvestmentAccountFields.SplitAmount:
                            ...
                            bt.SplitAmounts.Add(bt.SplitAmounts.Count, sEntry.Substring(1).ParseDecimalString(config));
