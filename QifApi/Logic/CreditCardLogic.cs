                ...
                if (sEntry.Length > 0)
                {
                    ...
                    switch (sEntry[0].ToString())
                    {
                        case NonInvestmentAccountFields.Date:
                            // Set the date value
                            cct.Date = sEntry.Substring(1).ParseDateString(config);

                            // Stop processing
                            break;
                        case NonInvestmentAccountFields.Amount:
                            // Set the amount value
                            cct.Amount = sEntry.Substring(1).ParseDecimalString(config);
                        ...
                        case NonInvestmentAccountFields.SplitAmount:
                            ...
                            cct.SplitAmounts.Add(cct.SplitAmounts.Count, sEntry.Substring(1).ParseDecimalString(config));
